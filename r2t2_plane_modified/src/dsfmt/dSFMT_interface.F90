...
private
public :: dSFMT_t, dSFMT_init, dSFMT_jumpf, dSFMT_end, dSFMT_reset,  &
...
interface
    ...
    pure subroutine dsfmt_jump_util_f(dsfmt_state, steps) bind(c, name='jump_dsfmt_t')
        import :: c_ptr, c_int32_t
        type(c_ptr), value, intent(in) :: dsfmt_state
        integer(c_int32_t), value, intent(in) :: steps
    end subroutine dsfmt_jump_util_f
...
!--- Initialisation, termination and state interaction ---
    ...
    !Jump N steps
    subroutine dSFMT_jumpf(rng,steps)
        type(dSFMT_t), intent(inout) :: rng
        integer, intent(in) :: steps
        call dsfmt_jump_util_f(rng%dSFMT_state,steps)
    end subroutine dSFMT_jumpf
